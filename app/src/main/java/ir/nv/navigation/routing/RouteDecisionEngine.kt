 ...
 *
 * The score combines base travel time, live/predicted traffic delay, distance
 * and a speed-sensitive energy index. Every metric is normalized across the
 * current alternatives, so no single unit dominates solely because of scale.
 */
object RouteDecisionEngine {
    ...
    fun evaluate(
        ...
        val rows = routes.mapIndexed { index, route ->
            Raw(
                ...
                trafficDelaySeconds = traffic.getOrNull(index)?.delaySeconds?.coerceAtLeast(0.0) ?: 0.0,
                distanceMeters = route.distanceMeters.coerceAtLeast(0.0),
                energyIndex = energyIndex(route)
            )
        }
        val timeRange = Range.of(rows.map(Raw::travelSeconds))
        ...
        return rows.map { row ->
            Evaluation(
                ...
                score = TIME_WEIGHT * timeRange.normalize(row.travelSeconds) +
    ...
    private fun energyIndex(route: Route): Double {
        ...
        // Rolling resistance is roughly distance-proportional; the quadratic
        // term is a conservative proxy for aerodynamic energy at higher speed.
