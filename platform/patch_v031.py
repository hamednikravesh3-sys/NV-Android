...
t = t.replace("versionCode = ver.V1", "versionCode = 31")
t = t.replace("versionName = ver.V2", "versionName = '0.31.0'")
...
t = t.replace('bar.setOnClickListener(v -> NvRuntimeController.openSearch(activity, ""));',
              'bar.setOnClickListener(v -> NvV031Actions.openSmartSearch(activity));', 1)
...
new_call = '''final int radiusMeters = NvV031Actions.getSearchRadius(activity, category.radiusMeters);
...
t = t.replace('text = "◎  GPS " + accuracy + "م";', 'text = "◎  GPS " + accuracy + "م";')
t = t.replace('text = "⚠  GPS " + accuracy + "م";', 'text = "⚠  GPS " + accuracy + "م";')
...
t = t.replace('loc.getAccuracy() > 50f', 'loc.getAccuracy() > 75f')
...
img = Image.open(BytesIO(icon)).convert('RGBA')
px = img.load()
...
img.save(splash_dir / 'nv_splash_logo.webp', 'WEBP', lossless=True, quality=100)
...
(root / 'NV_ENGINE_ATTRIBUTION.txt').write_text(
    'NV v0.31 uses Organic Maps/OpenStreetMap. ETA refreshes by movement/time; destination confidence avoids silent wrong picks; mixed trips use subway route-relation topology when available; cost inputs are configurable; no live traffic, train timetable or ride-hailing data is fabricated.\n',
    encoding='utf-8')

# Build-time assertions.
assert 'NvV031Actions.openSmartSearch' in (dst / 'NvMapMenuOverlay.java').read_text(encoding='utf-8')
assert 'NvV031Actions.openRouteAlerts' in (dst / 'NvMapMenuOverlay.java').read_text(encoding='utf-8')
...
assert 'returnHome(activity)' in (dst / 'NvMapMenuOverlay.java').read_text(encoding='utf-8')
assert 'fetchMetroNetwork' in (dst / 'NvV031Actions.java').read_text(encoding='utf-8')
assert 'shortestMetroPath' in (dst / 'NvV031Actions.java').read_text(encoding='utf-8')
assert 'onlineSourceLat' in (dst / 'NvV031Actions.java').read_text(encoding='utf-8')
assert 'taxi_per_km_toman' in (dst / 'NvV031Actions.java').read_text(encoding='utf-8')
...
print('NV v0.31 hardening integration applied')
