...
(root / 'NV_ENGINE_ATTRIBUTION.txt').write_text(
    'NV v0.31 uses Organic Maps/OpenStreetMap. ETA refreshes as the vehicle moves; destination search uses confidence checks; mixed trips can derive station/transfer estimates from OSM subway route relations; configurable cost estimates are labeled approximate; no live traffic, train, taxi availability, or price data is fabricated.\n',
    encoding='utf-8')
...
assert 'NvV031Actions.openHurry(activity)' in (dst / 'NvMapMenuOverlay.java').read_text(encoding='utf-8')
assert 'case 1 -> NvV031Actions.goHome(activity);' in (dst / 'NvMapMenuOverlay.java').read_text(encoding='utf-8')
...
print('NV v0.31 production-hardening integration applied')
